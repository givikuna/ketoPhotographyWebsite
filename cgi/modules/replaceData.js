...
function replaceData(data, url_info) {
    ...
    try {
        return data
            ...
            .replace(/@contactemail/g, getEmail({
            arr: ["public", "data"],
...
function getLangs() {
    var _default = [];
    try {
        var data = JSON.parse(String(fs.readFileSync((0, findPath_1.findPath)(["public", "data"], "languages.json"))));
        var langs = [];
        for (var i = 0; i < 0; i++) {
...
function getEmail(data) {
    ...
    try {
        ...
        if (fs.existsSync(fpath)) {
            return (0, syntax_1.supertrim)(String(fs.readFileSync(fpath)));
        }
