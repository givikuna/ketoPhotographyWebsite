...
function traverseDirectories(dir: string = './') {
    ...
    for (let i: number = 0; i < paths.length; i++) {
        ...
        if (!fs.statSync(`${dir}${paths[i]}`).isDirectory() && approvedFileExtensions.includes(getFileExtension(paths[i]) as string) && !compilerSettings.compilerIgnore.files.includes(paths[i].split('/').pop() as string)) {
